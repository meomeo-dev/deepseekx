...
# Default allowed domains - can be overridden with DEEPSEEK_ALLOWED_DOMAINS.
DEEPSEEK_ALLOWED_DOMAINS="${DEEPSEEK_ALLOWED_DOMAINS:-api.deepseek.com}"
...
CONTAINER_NAME="deepseekx_$(echo "$WORK_DIR" | sed 's/\//_/g' | sed 's/[^a-zA-Z0-9_-]//g')"
...
# Verify that DEEPSEEK_ALLOWED_DOMAINS is not empty
if [ -z "$DEEPSEEK_ALLOWED_DOMAINS" ]; then
  echo "Error: DEEPSEEK_ALLOWED_DOMAINS is empty."
  exit 1
fi
...
docker exec --user root "$CONTAINER_NAME" bash -c "mkdir -p /etc/deepseekx"
for domain in $DEEPSEEK_ALLOWED_DOMAINS; do
  # Validate domain format to prevent injection
  if [[ ! "$domain" =~ ^[a-zA-Z0-9][a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$ ]]; then
    echo "Error: Invalid domain format: $domain"
    exit 1
  fi
  echo "$domain" | docker exec --user root -i "$CONTAINER_NAME" bash -c "cat >> /etc/deepseekx/allowed_domains.txt"
done

# Set proper permissions on the domains file
docker exec --user root "$CONTAINER_NAME" bash -c "chmod 444 /etc/deepseekx/allowed_domains.txt && chown root:root /etc/deepseekx/allowed_domains.txt"
...
docker exec -it "$CONTAINER_NAME" bash -c "cd \"/app$WORK_DIR\" && deepseekx --sandbox workspace-write --ask-for-approval on-request ${quoted_args}"
