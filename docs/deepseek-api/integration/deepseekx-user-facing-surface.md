...
- 内部 crate 名、Rust 模块名、核心类型名和已有 wire schema 默认
  保留。
- 对外兼容入口应保留 Codex 别名，避免破坏现有脚本和上游同步。
...
- 新增 `deepseekx` 主 binary 或安装别名。
- 保留 `codex` 兼容 binary 或软链接。
- 用户帮助文本默认展示 `deepseekx`。
- 兼容提示可写成 `codex` 仍可用，但新文档和错误修复建议使用
  `deepseekx`。
...
用户配置面必须支持 DeepSeekX 命名，但不能立即移除 Codex 兼容。
...
- 保留读取 `CODEX_HOME` 和 `~/.codex` 作为兼容路径。
- 优先级建议为：
  `DEEPSEEKX_HOME` > `CODEX_HOME` > `~/.deepseekx` > legacy `~/.codex`。
- 首次运行若发现 legacy 配置，可提示迁移或继续兼容读取。
- 不应静默复制 secrets；auth 和 token 迁移必须有明确提示。
...
- OpenAI / ChatGPT 登录作为兼容 provider 能力保留。
- `deepseekx login --with-api-key` 的帮助示例使用
  `DEEPSEEK_API_KEY`。
...
- 兼容产物可附带 `codex` alias，但不作为 DeepSeekX 主入口。
...
- 增加 `DEEPSEEKX_HOME`。
- 默认使用 `~/.deepseekx`。
- 兼容读取 `CODEX_HOME` 和 `~/.codex`。
- 增加配置迁移提示，不静默移动 secret。
...
- 旧 `codex` 命令、`CODEX_HOME` 和 `~/.codex` 可作为兼容路径继续工作。
