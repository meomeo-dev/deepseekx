...
private_repo="${WE_PRIVATE_REPO:-meomeo-dev/deepseekx}"
public_repo="${WE_PUBLIC_REPO:-meomeo-dev/deepseekx}"
...
check_repo() {
  ...
  echo "repo=$repo"
  echo "branch=$branch"
  branch_path="$(branch_api_path)"

  local protection_output
  api_error="$(mktemp)"
  if protection_output="$(gh api "repos/${repo}/branches/${branch_path}/protection" \
    --jq '{
