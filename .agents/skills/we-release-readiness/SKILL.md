...
验证、公开仓库安全检查、tag、GitHub Release 页面和用户确认。
...
- 判断是否需要 `$we-public-sync` 检查公开仓库安全边界。
...
- 不执行公开仓库安全修复；公开检查由 `$we-public-sync` 执行。
...
- 是否涉及公开仓库安全边界；涉及时必须进入 `$we-public-sync`。
...
- Blocked：存在版本、dirty state、CI、包内容、凭据或公开安全阻塞。
