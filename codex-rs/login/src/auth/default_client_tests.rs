...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("deepseekx-tui"), true);
    assert_eq!(is_first_party_originator("deepseekx_vscode"), true);
    assert_eq!(is_first_party_originator("DeepSeekX Something Else"), true);
    assert_eq!(is_first_party_originator("codex-tui"), false);
...
fn is_first_party_chat_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_chat_originator("deepseekx_vscode"), false);
}
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "deepseekx_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "deepseekx_cli_rs/0.0.0 (bad_suffix)"
    );
}
