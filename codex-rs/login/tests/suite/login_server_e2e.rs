...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to DeepSeekX ChatGPT login"),
        "error body should clearly explain the DeepSeekX access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to DeepSeekX to retry"),
    ...
    assert!(
        !body.contains("get access to DeepSeekX"),
