WARNING: this code is mainly generated and should not be used in production

# deepseekx-debug-client

A tiny interactive client for `deepseekx app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `deepseekx app-server` itself):
...
- `--codex-bin <path>`: path to the `deepseekx` binary (default: `deepseekx`).
- `-c, --config key=value`: pass through `--config` overrides to `deepseekx`.
