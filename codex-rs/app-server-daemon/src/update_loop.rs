...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed DeepSeekX binary {}",
...
async fn install_latest_standalone() -> Result<()> {
    anyhow::bail!(
        "DeepSeekX managed standalone updater is not configured; refusing to run the OpenAI Codex updater"
    )
}
