...
const CHAT_WIRE_API_REMOVED_ERROR: &str = concat!(
    "`wire_api = \"chat\"` is no longer supported.\n",
    "How to fix: set `wire_api = \"responses\"` for the Responses API, ",
    "or `wire_api = \"chat_completions\"` for the bounded ",
...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// Chat Completions API at `/chat/completions`.
...
impl ModelProviderInfo {
    ...
    pub fn create_deepseek_provider() -> ModelProviderInfo {
        Self::create_deepseek_provider_with(/*base_url*/ None)
    }

    fn create_deepseek_provider_with(base_url: Option<String>) -> ModelProviderInfo {
        let base_url = base_url.unwrap_or_else(|| DEEPSEEK_DEFAULT_BASE_URL.into());
        ModelProviderInfo {
...
pub fn built_in_model_providers(
    ...
    built_in_model_providers_with_base_urls(openai_base_url, /*deepseek_base_url*/ None)
}

pub fn built_in_model_providers_with_base_urls(
    openai_base_url: Option<String>,
    ...
    let deepseek_provider = P::create_deepseek_provider_with(deepseek_base_url);

    // We keep third-party built-ins limited to providers that need Codex-owned
    // protocol or capability adaptation.
    [
