...
mod tests {
    ...
    #[test]
    fn model_provider_proto_roundtrips_chat_completions_wire_api() {
        let expected = ModelProviderInfo {
            wire_api: WireApi::ChatCompletions,
            ..expected_provider()
        };
        let proto = model_provider_to_proto("deepseek", expected.clone());
        let result = model_provider_from_proto(proto);
        let (id, actual) = result.expect("model provider from proto");
