...
#[cfg(unix)]
const DEEPSEEKX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/deepseekx/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/deepseekx/managed_config.toml`.
...
pub(super) fn managed_config_default_path(codex_home: &Path) -> PathBuf {
    ...
    {
        let _ = codex_home;
        PathBuf::from(DEEPSEEKX_MANAGED_CONFIG_SYSTEM_PATH)
    }
