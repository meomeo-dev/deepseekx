...
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $DEEPSEEKX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the DeepSeekX app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "DeepSeekX", version, about = "Bootstrap DeepSeekX app-server", long_about = None)]
struct Cli {
    /// Path to the `deepseekx` CLI binary. When set, requests use stdio by
    /// spawning `deepseekx app-server` as a child process.
    #[arg(long, env = "DEEPSEEKX_BIN", global = true)]
    ...
    /// Forwarded to the `deepseekx` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `deepseekx app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `deepseekx app-server --listen`.
    ...
    /// Send a user message through the DeepSeekX app-server.
    SendMessage {
        /// User message to send to DeepSeekX.
        user_message: String,
    },
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codex_bin = codex_bin.unwrap_or_else(|| PathBuf::from("deepseekx"));
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started deepseekx app-server");
...
impl CodexClient {
    ...
    fn spawn_stdio(codex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("deepseekx app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("deepseekx app-server stdout unavailable")?;
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to deepseekx app-server")?;
                    return Ok(());
                }
                bail!("deepseekx app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from deepseekx app-server")?;
                if bytes == 0 {
                    bail!("deepseekx app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[deepseekx app-server exited: {status}]");
            return;
        }
