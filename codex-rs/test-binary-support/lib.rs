...
pub struct TestBinaryDispatchGuard {
    ...
    _previous_deepseekx_home: Option<std::ffi::OsString>,
}
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let codex_home = match tempfile::Builder::new().prefix(codex_home_prefix).tempdir() {
                ...
                Err(error) => panic!("failed to create test DEEPSEEKX_HOME: {error}"),
            };
            let previous_deepseekx_home = std::env::var_os("DEEPSEEKX_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("DEEPSEEKX_HOME", codex_home.path());
            }
            ...
            match previous_deepseekx_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("DEEPSEEKX_HOME", value);
                },
                None => unsafe {
                    std::env::remove_var("DEEPSEEKX_HOME");
                },
            }
            ...
            Some(TestBinaryDispatchGuard {
                ...
                arg0,
                _previous_deepseekx_home: previous_deepseekx_home,
            })
        }
    }
}
