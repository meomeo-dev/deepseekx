...
pub const DEEPSEEKX_EXEC_SERVER_URL_ENV_VAR: &str = "DEEPSEEKX_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the DeepSeekX runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor reads `DEEPSEEKX_EXEC_SERVER_URL`
/// while provider-based construction accepts a provider-supplied snapshot.
///
/// Setting `DEEPSEEKX_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `DEEPSEEKX_EXEC_SERVER_URL` and local runtime paths
    /// used when creating local filesystem helpers.
    pub async fn new(args: EnvironmentManagerArgs) -> Self {
        let EnvironmentManagerArgs {
            local_runtime_paths,
        } = args;
        let exec_server_url = std::env::var(DEEPSEEKX_EXEC_SERVER_URL_ENV_VAR).ok();
        Self::from_default_provider_url(exec_server_url, local_runtime_paths).await
    }

    /// Builds a manager from `DEEPSEEKX_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `DEEPSEEKX_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves `DEEPSEEKX_EXEC_SERVER_URL` behavior.
    ...
    /// Builds a manager from the DeepSeekX environment-variable provider without
    /// reading user config files from `DEEPSEEKX_HOME`.
...
impl Environment {
    /// Builds an environment from the raw `DEEPSEEKX_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `DEEPSEEKX_EXEC_SERVER_URL` value and
