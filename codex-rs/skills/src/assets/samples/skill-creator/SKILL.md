...
- "Where should I create this skill? If you do not have a preference, I will place it in `$DEEPSEEKX_HOME/skills` (or `~/.deepseekx/skills` when `DEEPSEEKX_HOME` is unset) so DeepSeekX can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$DEEPSEEKX_HOME/skills`; when `DEEPSEEKX_HOME` is unset, fall back to `~/.deepseekx/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${DEEPSEEKX_HOME:-$HOME/.deepseekx}/skills"
scripts/init_skill.py my-skill --path "${DEEPSEEKX_HOME:-$HOME/.deepseekx}/skills" --resources scripts,references
