...
Set a stable path to the skill CLI (default `DEEPSEEKX_HOME` is `~/.deepseekx`):

```
export DEEPSEEKX_HOME="${DEEPSEEKX_HOME:-$HOME/.deepseekx}"
export IMAGE_GEN="$DEEPSEEKX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$DEEPSEEKX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
