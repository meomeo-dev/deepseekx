...
//!
//! If the keyring is not available or fails, we fall back to the DeepSeekX home
//! `.credentials.json` file, consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create DEEPSEEKX_HOME temp dir");
            unsafe {
                std::env::set_var("DEEPSEEKX_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
