...
async fn responses_mode_stream_cli() {
    ...
    let bin = codex_utils_cargo_bin::cargo_bin("deepseekx").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("DEEPSEEKX_HOME", home.path())
...
/// Verify that `deepseekx exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("deepseekx").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("DEEPSEEKX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");
