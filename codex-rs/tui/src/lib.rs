...
#[cfg(unix)]
const AUTO_CONNECT_DAEMON_TIMEOUT: std::time::Duration = std::time::Duration::from_millis(50);
...
#[cfg(unix)]
async fn maybe_connect_default_daemon_app_server(
    codex_home: &Path,
) -> Option<(AbsolutePathBuf, AppServerClient)> {
    ...
    match tokio::time::timeout(
        AUTO_CONNECT_DAEMON_TIMEOUT,
        connect_remote_app_server(endpoint),
    )
    .await
    {
        Ok(Ok(app_server)) => Some((socket_path, app_server)),
        Ok(Err(err)) => {
        ...
        Err(_) => {
            tracing::debug!(
                ...
                timeout_ms = AUTO_CONNECT_DAEMON_TIMEOUT.as_millis(),
...
pub async fn run_main(
    ...
    let (remote_endpoint, preconnected_app_server) = match explicit_remote_endpoint {
        Some(endpoint) => (Some(endpoint), None),
        None => match maybe_connect_default_daemon_app_server(&codex_home).await {
            Some((socket_path, app_server)) => (
...
async fn run_ratatui_app(
    ...
    let app_server_session = match preconnected_app_server {
        Some(app_server) => AppServerSession::new(app_server),
...
mod tests {
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_connect_default_daemon_app_server(codex_home.path())
                .await
                .is_none()
        );
        Ok(())
