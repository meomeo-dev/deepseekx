...
mod tests {
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.deepseekx/config.toml").abs();
        let dot_codex_folder = test_path_buf("/tmp/worktree/.deepseekx").abs();
        ...
        assert_eq!(
            super::hook_metadata_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.meomeo.deepseekx".to_string(),
